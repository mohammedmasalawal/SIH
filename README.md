...
| `fire_near_infrastructure` | a **one-off** detection (`recurrence_count` ≤ `INFRA_MAX_RECURRENCE` = 1 — the cell's first active day in the 90-day look-back), **any label**, within `INFRA_FIRE_MAX_DIST_M` of a critical facility: GEM refinery, chemical, thermal (oil/gas, coal) power, LNG terminal, oil/gas field, cement or steel plant (`INFRA_GEM_FACILITY_TYPES`), OSM `industrial=refinery` (`INFRA_OSM_INDUSTRIAL_TAGS`), or OSM `power=plant` whose `plant:source` includes coal/gas/oil/diesel/nuclear (`INFRA_OSM_POWER_SOURCES` — solar, wind, hydro, biomass, waste and untagged plants never count) | 1 day, 2 km | facility name, kind, distance |
...
| `large_fire_event` | `LARGE_FIRE_MIN_DETECTIONS`+ `agricultural burning`/`wildfire` detections (`LARGE_FIRE_LABELS`) within `LARGE_FIRE_RADIUS_M` on the same day (DBSCAN, so a cluster can chain beyond 5 km), after dropping detections in cells active on more than `LARGE_FIRE_PERSISTENT_MAX_ACTIVE_DAYS` of the previous `LARGE_FIRE_PERSISTENT_WINDOW_DAYS` (persistent industrial sites, coal-seam fires); judged only on complete UTC days | 10 within 5 km; >5 of 30 days = persistent | detection count, dominant class, total FRP |

What the September 2026 backfill (1–23 Sep) says about these starting values — 290 alerts after the re-score (was 504 under the first definitions):

- `industrial_anomaly`: 110, unchanged (includes both September gas-flare alerts, Mangaluru 17.1× and Hazira 4.8×).
- `fire_near_infrastructure`: 167, at 48 facilities (OSM coal plants 102, refineries 43, gas plants 21, one GEM cement plant). **None is a crop or wildfire detection**: those labels require no OSM industrial-context feature within 2 km, and that context contains every OSM refinery and non-renewable power plant — so in practice this alert flags *new hotspot cells at or inside refineries and thermal plants* (112 `industrial`, 55 `unknown`, median 360 m, many 0 m inside the facility outline), e.g. Panipat Refinery, MRPL, Rourkela and Angul captive power plants. Only a GEM facility point (not in the OSM context) could pair with a crop/wildfire label.
- `large_fire_event`: 10, all `wildfire`/`agricultural burning` (was 263, mostly industrial). **The persistence filter is per 375 m cell, so it does not fully remove coal-seam fires**: 3 of the 10 are in the Jharia coalfield (23.70–23.72 N, 86.42 E, labelled `wildfire` by landcover), where burning moves between neighbouring cells. Checking persistence over a neighbourhood (e.g. 1–2 km) instead of one cell is the obvious next step.
