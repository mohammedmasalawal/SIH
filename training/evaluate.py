...
Reproduces the model's own held-out spatial-block+temporal set deterministically
(same CSV + same random_seed recorded in the artifact's metadata, via the same
prepare_training_frame() train.py used), rather than persisting a second
held-out-rows file -- since a grid cell never straddles train/test, and gold cells
are excluded the same way on both sides, nothing about that reproduction can leak
train or gold rows into the score.
"""
...
from training.train_model import prepare_training_frame
...
def evaluate(
    ...
    safe_frame, _, test_idx, cutoff_date = prepare_training_frame(labeled_csv, gold_csv=gold_csv, seed=seed)
    if len(test_idx) == 0:
